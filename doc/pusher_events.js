...
var bw_event_hash = {
  "job:started" : '{'
    + '"build_id": 468459,'
    + '"id": 46,'
    + '"started_at": "2013-01-02T15:30:00Z",'
    + '"state": "started"'
    + '}'
  ,
  ...
  "job:finished" : '{'
    + '"build_id": 468459,'
    + '"finished_at": "2013-01-02T15:33:40Z",'
    + '"id": 46,'
    + '"result": 0,'
    + '"state": "finished",'
    + '"status": 0'
  ...
  "build:started" : '{'
    + '"build": {'
        ...
        + '"committed_at": "2013-01-02T15:30:14Z",'
      ...
      + '"repository": {'
        ...
        + '"last_build_started_at": "2013-01-02T15:30:00Z",'
        + '"slug": "bergie/create"'
        + '},'
      + '"started_at": "2013-01-02T15:30:00Z"'
    + '}'
  ...
  "build:finished": '{'
    ...
    + '"repository": {'
      ...
      + '"last_build_finished_at": "2013-01-02T15:33:40Z",'
      + '"last_build_id": 468459,'
      + '"last_build_number": "11",'
      + '"last_build_result": 0,'
      + '"last_build_started_at": "2013-01-02T15:30:00Z",'
...
var bw_append_log_event = function(event_name, json_string, channel) {
  ...
  jQuery('#bw_event_' + safe_name).click(function(){
    ...
    for (var i = 0; i < iteration_counter; i++) {
      dataString = dataString + json_string;
    }
    jQuery('#event_channel').val(channel);
    jQuery('#event_event_name').val(event_name);
    jQuery('#event_data').val(dataString);
...
for (event_name in job_46_hash) {
  bw_append_log_event(event_name, job_46_hash[event_name], 'job-46');
}
